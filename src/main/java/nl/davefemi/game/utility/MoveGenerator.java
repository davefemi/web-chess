...
public final class MoveGenerator {
    ...
    public static List<SingleMove> generateMoves(Board board, PieceColor color, boolean isActiveColor) throws BoardException {
        ...
        moves.addAll(getPawnMoves(board, BoardScanner.getCurrentPiecePositions(board, PieceType.PAWN, color), enemyColor, isActiveColor));
        return moves;
    }
    ...
    private static List<SingleMove> getPawnMoves(Board board, List<Position> positions, PieceColor enemyColor, boolean isActiveColor) throws BoardException {
        List<SingleMove> pseudoMoves = new ArrayList<>();
        if (!positions.isEmpty()) {
            for (Position position : positions) {
